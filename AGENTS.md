...
- Mass-creation abuse is blunted by the **link cap** (task 6): max 10 links per owner key — `countOwnerLinks` COUNT check before insert, 429 "Link limit reached (10). Delete one to add another." at the cap; deleting a link frees a slot. Karim chose this over Upstash rate limiting (no new infra, no billing). Known accepted gap: an attacker minting fresh owner keys can exceed it globally.
...
- [x] Task 6 — Abuse protection (2026-08-17): Karim chose a **10-link cap per owner** over Upstash rate limiting (no new infra/billing; the two @upstash packages were uninstalled, dependency list back to just @neondatabase/serverless). `countOwnerLinks` COUNT check before insert → 429 with a delete-one message, which the create form now surfaces verbatim (was swallowed by a generic catch before). Same commit: URL ≤ 2048 chars + streamed body ≤ 8KB guards (both were unbounded — with the cap, 10 × 4MB URLs per key was still storage abuse). Known accepted gap: an attacker minting fresh owner keys can exceed the cap globally. Verified: 32/32 tests, `vercel dev` full flow (10×200, 11th → 429, delete frees a slot, second owner unaffected), probe rows cleaned up
